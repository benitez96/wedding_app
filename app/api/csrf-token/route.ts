...
export async function GET(request: NextRequest) {
  try {
    ...
    // Agregar headers CORS si es necesario (solo en producción y con origin válido)
    if (process.env.NODE_ENV === "production" && origin) {
      const allowedOrigins =
        process.env.CORS_ORIGIN?.split(",").filter(Boolean) || [];

      // Solo setear CORS headers si el origin está en la whitelist
...
export async function OPTIONS(request: NextRequest) {
  ...
  // Solo setear CORS headers en producción y con origin válido
  if (process.env.NODE_ENV === "production" && origin) {
    const allowedOrigins =
      process.env.CORS_ORIGIN?.split(",").filter(Boolean) || [];

    // Solo setear headers si el origin está en la whitelist
    if (allowedOrigins.length > 0 && allowedOrigins.includes(origin)) {
      response.headers.set("Access-Control-Allow-Origin", origin);
      response.headers.set("Access-Control-Allow-Credentials", "true");
      response.headers.set(
