...
show_menu() {
    ...
    local default_selected=(true false false false)

    if command -v gum >/dev/null 2>&1; then
        local selected
        selected=$(printf "%s\n" "${options[@]}" | gum choose --no-limit --selected "Claude Code" --header "Which AI assistants do you use?") || true

        local chosen=(false false false false)
        while IFS= read -r choice; do
            case "$choice" in
                "Claude Code") chosen[0]=true ;;
                "Gemini CLI") chosen[1]=true ;;
                "Codex (OpenAI)") chosen[2]=true ;;
                "GitHub Copilot") chosen[3]=true ;;
            esac
        done <<< "$selected"

        if [ -z "$selected" ]; then
            chosen=(${default_selected[@]})
        fi

        SETUP_CLAUDE=${chosen[0]}
        SETUP_GEMINI=${chosen[1]}
        SETUP_CODEX=${chosen[2]}
        SETUP_COPILOT=${chosen[3]}
