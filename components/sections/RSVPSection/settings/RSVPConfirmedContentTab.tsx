...
interface RSVPConfirmedContentTabProps {
...
export function RSVPConfirmedContentTab({
  settings,
  onChange,
}: RSVPConfirmedContentTabProps) {
  const content = settings.confirmedContent ?? {
  ...
  function update(field: keyof typeof content, value: string) {
    onChange((prev) => ({
      ...prev,
      confirmedContent: { ...content, [field]: value },
    }));
  }
      ...
      <CardBody className="space-y-3">
        ...
        <SectionIconSelector
          ...
          onChange={(value) =>
            onChange((prev) => ({
              ...prev,
              confirmedContent: { ...content, icon: value },
            }))
          }
